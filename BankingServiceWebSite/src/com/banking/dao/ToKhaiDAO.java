...
public class ToKhaiDAO extends DAO{
	...
	public ArrayList<ToKhai> getCurrentToKhai(int customerID) {
		ArrayList<ToKhai> tokhai = new ArrayList<ToKhai>();
		String sql = "SELECT * FROM `tokhai` as TK INNER JOIN `user` as U WHERE U.ID = ? AND TK.userID = U.ID AND TK.enable = 1 ;";
		try {
	...
	public ArrayList<ToKhai> getTKByID(int tkID) {
		ArrayList<ToKhai> tokhai = new ArrayList<ToKhai>();
	...
	public boolean updateToKhai(ToKhai tk) {
		boolean kq = false;
		String sql= "UPDATE `tokhai` SET `Nghenghiep` = ?,"
				+"`Chucvu` = ?,"+"`Diachicongty` =?," +"`Tencongty` =?,"
				+"`Mucluong` = ?," + "`Chitieuhangthang`=?"
				+ "  WHERE ID = ?;";
		try (PreparedStatement pre = connection.prepareStatement(sql)) {
